...
// Licensed under the Defold License version 1.0 (the "License"); you may not
...
DM_PROPERTY_U32(rmtp_SpineComponents, 0, FrameReset, "# spine components",
                &rmtp_Spine);
...
static void
ResourceReloadedCallback(const dmResource::ResourceReloadedParams &params);
...
struct SpineModelContext {
  SpineModelContext() { memset(this, 0, sizeof(*this)); }
  dmResource::HFactory m_Factory;
...
dmGameObject::CreateResult
CompSpineModelNewWorld(const dmGameObject::ComponentNewWorldParams &params) {
  ...
  uint32_t comp_count = dmMath::Min(params.m_MaxComponentInstances,
                                    context->m_MaxSpineModelCount);
  ...
  dmGraphics::HVertexStreamDeclaration stream_declaration =
      dmGraphics::NewVertexStreamDeclaration(context->m_GraphicsContext);
  dmGraphics::AddVertexStream(stream_declaration, "position", 3,
                              dmGraphics::TYPE_FLOAT, false);
  dmGraphics::AddVertexStream(stream_declaration, "texcoord0", 2,
                              dmGraphics::TYPE_FLOAT, true);
  dmGraphics::AddVertexStream(stream_declaration, "color", 4,
                              dmGraphics::TYPE_FLOAT, true);
  dmGraphics::AddVertexStream(stream_declaration, "page_index", 1,
                              dmGraphics::TYPE_FLOAT, false);

  world->m_VertexDeclaration = dmGraphics::NewVertexDeclaration(
      context->m_GraphicsContext, stream_declaration);
  world->m_VertexBuffer =
      dmGraphics::NewVertexBuffer(context->m_GraphicsContext, 0, 0x0,
  ...
  *params.m_World = world;
  ...
  dmResource::RegisterResourceReloadedCallback(context->m_Factory,
                                               ResourceReloadedCallback, world);
...
dmGameObject::CreateResult CompSpineModelDeleteWorld(
  ...
  dmResource::UnregisterResourceReloadedCallback(
      ((SpineModelContext *)params.m_Context)->m_Factory,
      ResourceReloadedCallback, world);
...
static inline dmRender::HMaterial
GetMaterial(const SpineModelComponent *component) {
  return component->m_Material ? component->m_Material
                               : component->m_Resource->m_Material->m_Material;
}
...
static void ReHash(SpineModelComponent *component) {
  ...
  dmGameSystem::TextureSetResource *texture_set =
      resource->m_SpineScene->m_TextureSet;
...
static void SetTransformFromBone(dmGameObject::HInstance instance,
                                 const dmTransform::Transform &parent,
                                 const spBone *bone) {
  ...
  dmTransform::Transform local = dmTransform::Transform(
      dmVMath::Vector3(bone->worldX, bone->worldY, 0),
      dmVMath::Quat::rotationZ(radians), dmVMath::Vector3(sx, sy, 1));
...
static bool CreateGOBone(SpineModelComponent *component,
                         dmGameObject::HCollection collection,
                         dmGameObject::HInstance goparent, spBone *parent,
                         spBone *bone, int indent) {
  ...
  dmGameObject::Result result =
      dmGameObject::SetIdentifier(collection, bone_instance, id);
  ...
  component->m_BoneNameToNodeInstanceIndex.Put(
      name_hash, component->m_BoneInstances.Size());
  ...
  for (int n = 0; n < bone->childrenCount; ++n) {
    // Since I haven't figured out how to update the local transforms, we do it
    // in world space However, that means all bones are parented to the top game
    // object

    if (!CreateGOBone(component, collection, component->m_Instance, bone,
                      bone->children[n], indent + 2))
      // if (!CreateGOBone(component, collection, bone_instance, bone,
      // bone->children[n], indent + 2))
      return false;
  }
  return true;
...
static bool CreateGOBones(SpineModelWorld *world,
                          SpineModelComponent *component) {
  // dmGameObject::HCollection collection =
  // dmGameObject::GetCollection(component->m_Instance);
  ...
  component->m_BoneNameToNodeInstanceIndex.SetCapacity(
      (skeleton->bonesCount + 1) / 2, skeleton->bonesCount);
  if (!CreateGOBone(component,
                    dmGameObject::GetCollection(component->m_Instance),
                    component->m_Instance, 0, skeleton->root, 0)) {
    dmLogError("Failed to create bones");
    dmGameObject::DeleteBones(
        component->m_Instance); // iterates recursively and deletes the ones
                                // marked as a bone
...
static inline SpineModelComponent *GetComponentFromIndex(SpineModelWorld *world,
                                                         int index) {
  return world->m_Components.Get(index);
}

static void *
CompSpineModelGetComponent(const dmGameObject::ComponentGetParams &params) {
  SpineModelWorld *world = (SpineModelWorld *)params.m_World;
  uint32_t index = (uint32_t)(uintptr_t)params.m_UserData;
  return GetComponentFromIndex(world, index);
}

static inline uint32_t FindAnimationIndex(SpineModelComponent *component,
                                          dmhash_t animation) {
  SpineModelResource *spine_model = component->m_Resource;
  SpineSceneResource *spine_scene = spine_model->m_SpineScene;
...
static SpineAnimationTrack *GetTrackFromIndex(SpineModelComponent *component,
                                              int track_index) {
...
static bool PlayAnimation(SpineModelComponent *component, dmhash_t animation_id,
                          dmGameObject::Playback playback, float blend_duration,
                          float offset, float playback_rate, int track_index) {
  ...
  if (index >= spine_scene->m_Skeleton->animationsCount) {
    dmLogError(
        "No animation index %u is too large. Number of animations are %u",
        index, spine_scene->m_Skeleton->animationsCount);
    return false;
  }
  ...
  track.m_AnimationInstance = spAnimationState_setAnimation(
      component->m_AnimationStateInstance, track_index, animation, loop);
  ...
  track.m_AnimationInstance->trackTime =
      dmMath::Clamp(offset, track.m_AnimationInstance->animationStart,
                    track.m_AnimationInstance->animationEnd);
...
static void CancelTrackAnimation(SpineModelComponent *component,
                                 int32_t track_index) {
  SpineAnimationTrack *track = GetTrackFromIndex(component, track_index);
  if (!track || !track->m_AnimationInstance)
    return;

  spAnimationState_clearTrack(component->m_AnimationStateInstance,
                              track->m_AnimationInstance->trackIndex);
...
static bool GetSender(SpineModelComponent *component,
                      dmMessage::URL *out_sender) {
  dmMessage::URL sender;
  sender.m_Socket = dmGameObject::GetMessageSocket(
      dmGameObject::GetCollection(component->m_Instance));
  if (dmMessage::IsSocketValid(sender.m_Socket)) {
    dmGameObject::Result go_result = dmGameObject::GetComponentId(
        component->m_Instance, component->m_ComponentIndex, &sender.m_Fragment);
...
static void SendAnimationDone(SpineModelComponent *component,
                              const spAnimationState *state,
                              const spTrackEntry *entry, const spEvent *event) {
  ...
  if (!GetSender(component, &sender)) {
    dmLogError("Could not send animation_done to listener because of "
  ...
  if (track.m_CallbackInfo) {
    ...
    RunTrackCallback(track.m_CallbackInfo,
                     dmGameSystemDDF::SpineAnimationDone::m_DDFDescriptor,
    ...
    // Therefore, we need to check whether we are going to remove the same
    // callback that we are running. If not, it has already been removed.
    ...
    dmGameObject::Result result =
        dmGameObject::PostDDF(&message, &sender, &receiver, 0, true);
...
static void SendSpineEvent(SpineModelComponent *component,
                           const spAnimationState *state,
                           const spTrackEntry *entry, const spEvent *event) {
  SpineAnimationTrack &track = component->m_AnimationTracks[entry->trackIndex];
  ...
  message.m_String =
      dmHashString64(event->stringValue ? event->stringValue : "");
  ...
  if (track.m_CallbackInfo) {
    RunTrackCallback(track.m_CallbackInfo,
                     dmGameSystemDDF::SpineEvent::m_DDFDescriptor,
                     (const char *)&message, &sender);
  } else {
    dmGameObject::Result result =
        dmGameObject::PostDDF(&message, &sender, &receiver, 0, false);
    if (result != dmGameObject::RESULT_OK) {
      dmLogError("Could not send animation event '%s' from animation '%s' to "
                 "listener: %d",
                 entry->animation->name, event->data->name, result);
    }
  }
}
...
static void SpineEventListener(spAnimationState *state, spEventType type,
  ...
  // http://esotericsoftware.com/spine-api-reference#AnimationStateListener
  switch (type) {
  // case SP_ANIMATION_START:
  // {
  //     printf("Animation %s started on track %i\n", entry->animation->name,
  //     entry->trackIndex); break;
  // }
  // case SP_ANIMATION_INTERRUPT:
  // {
  //     printf("Animation %s interrupted on track %i\n",
  //     entry->animation->name, entry->trackIndex); break;
  // }
  ...
  case SP_ANIMATION_COMPLETE: {
    if (entry->mixingTo != 0) {
      ...
      // The problem is that the track is already owned by the new animation,
      // and we cannot call the previous callback (as we haven't stored it)
      return;
    }

    SpineAnimationTrack &track =
        component->m_AnimationTracks[entry->trackIndex];
  ...
  case SP_ANIMATION_DISPOSE: {
    SpineAnimationTrack *track =
        GetTrackFromIndex(component, entry->trackIndex);
...
dmGameObject::CreateResult
CompSpineModelCreate(const dmGameObject::ComponentCreateParams &params) {
  SpineModelWorld *world = (SpineModelWorld *)params.m_World;

  if (world->m_Components.Full()) {
    dmLogError(
        "Spine Model could not be created since the buffer is full (%d).",
  ...
  component->m_Transform = dmTransform::Transform(
      Vector3(params.m_Position), params.m_Rotation, params.m_Scale);
  ...
  if (0 == spSkeleton_setSkinByName(component->m_SkeletonInstance,
                                    component->m_Resource->m_Ddf->m_Skin)) {
    spSkeleton_setSkin(component->m_SkeletonInstance,
                       spine_scene->m_Skeleton->defaultSkin);
  }
  spSkeleton_setSlotsToSetupPose(component->m_SkeletonInstance);

  component->m_AnimationStateInstance =
      spAnimationState_create(spine_scene->m_AnimationStateData);
  ...
  // We need to make sure that bone GOs are created before we start the default
  // animation.
  if (spine_model->m_CreateGoBones) {
    if (!CreateGOBones(world, component)) {
      dmLogError(
          "Failed to create game objects for bones in spine model. Consider "
          "increasing collection max instances (collection.max_instances).");
      DestroyComponent(world, index);
      return dmGameObject::CREATE_RESULT_UNKNOWN_ERROR;
    }
  }
  dmhash_t animation_id =
      dmHashString64(component->m_Resource->m_Ddf->m_DefaultAnimation);
  PlayAnimation(component, animation_id, dmGameObject::PLAYBACK_LOOP_FORWARD,
                0.0f, component->m_Resource->m_Ddf->m_Offset,
                component->m_Resource->m_Ddf->m_PlaybackRate, 0);
...
static void DestroyComponent(SpineModelWorld *world, uint32_t index) {
  ...
  // If we're going to use memset, then we should explicitly clear pose and
  // instance arrays.
...
dmGameObject::CreateResult
CompSpineModelDestroy(const dmGameObject::ComponentDestroyParams &params) {
...
dmGameObject::CreateResult CompSpineModelAddToUpdate(
...
static void ApplyIKTargets(SpineModelComponent *component) {
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    dmVMath::Vector3 model_pos = (dmVMath::Vector3)dmTransform::Apply(
        dmTransform::Inv(dmTransform::Mul(
            dmGameObject::GetWorldTransform(component->m_Instance),
...
dmGameObject::UpdateResult
CompSpineModelUpdate(const dmGameObject::ComponentsUpdateParams &params,
                     dmGameObject::ComponentsUpdateResult &update_result) {
  SpineModelWorld *world = (SpineModelWorld *)params.m_World;

  float dt = params.m_UpdateContext->m_DT;

  dmArray<SpineModelComponent *> &components =
      world->m_Components.GetRawObjects();
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    const Matrix4 &go_world =
        dmGameObject::GetWorldMatrix(component.m_Instance);
    ...
    spAnimationState_apply(component.m_AnimationStateInstance,
                           component.m_SkeletonInstance);
    ...
    if (component.m_ReHash ||
        (component.m_RenderConstants && dmGameSystem::AreRenderConstantsUpdated(
                                            component.m_RenderConstants))) {
      ReHash(&component);
    }
  ...
  // Since we've moved the child game objects (bones), we need to sync back the
  // transforms
  update_result.m_TransformsUpdated = num_active > 0;
  return dmGameObject::UPDATE_RESULT_OK;
}

static inline dmGameSystemDDF::SpineModelDesc::BlendMode
SpineBlendModeToRenderBlendMode(spBlendMode sp_blend_mode) {
...
static void
FillRenderObject(SpineModelWorld *world,
                 dmRender::HRenderContext render_context,
                 dmRender::RenderObject &ro,
                 dmGameSystem::HComponentRenderConstants constants,
                 dmGraphics::HTexture texture, dmRender::HMaterial material,
...
static void RenderBatch(SpineModelWorld *world,
                        dmRender::HRenderContext render_context,
                        dmRender::RenderListEntry *buf, uint32_t *begin,
                        uint32_t *end) {
  ...
  const SpineModelComponent *first =
      (const SpineModelComponent *)components[component_index];
  const SpineModelResource *resource = first->m_Resource;

  dmGameSystemDDF::SpineModelDesc::BlendMode blend_mode =
      resource->m_Ddf->m_BlendMode;
  bool use_inherit_blend =
      blend_mode == dmGameSystemDDF::SpineModelDesc::BLEND_MODE_INHERIT;

  uint32_t vertex_start = world->m_VertexBufferData.Size();
  uint32_t vertex_count = 0;
  uint32_t draw_desc_buffer_count = 0;

  // This is a temporary scratch buffer just used for this batch call, so we
  // make sure to reset it.
  world->m_DrawDescBuffer.SetSize(0);

  for (uint32_t *i = begin; i != end; ++i) {
    component_index = (uint32_t)buf[*i].m_UserData;
    const SpineModelComponent *component =
        (const SpineModelComponent *)components[component_index];
    vertex_count += dmSpine::CalcVertexBufferSize(component->m_SkeletonInstance,
                                                  world->m_SkeletonClipper, 0);

    if (use_inherit_blend) {
      draw_desc_buffer_count +=
          dmSpine::CalcDrawDescCount(component->m_SkeletonInstance);
    }
  }

  if (draw_desc_buffer_count &&
      draw_desc_buffer_count > world->m_DrawDescBuffer.Capacity()) {
  ...
  for (uint32_t *i = begin; i != end; ++i) {
    ...
    const SpineModelComponent *component =
        (const SpineModelComponent *)components[component_index];
    vertex_count += dmSpine::GenerateVertexData(
        world->m_VertexBufferData, component->m_SkeletonInstance,
        world->m_SkeletonClipper, component->m_World,
        use_inherit_blend ? &world->m_DrawDescBuffer : 0);
  }

  dmGraphics::HTexture texture =
      resource->m_SpineScene->m_TextureSet->m_Texture
          ->m_Texture; // spine - texture set resource - texture resource -
                       // texture
  ...
  if (use_inherit_blend) {
    ...
    if (draw_desc_count > 0) {
      ...
      world->m_RenderObjects.SetSize(world->m_RenderObjects.Size() +
                                     merged_size);

      for (int i = 0; i < merged_size; ++i) {
        dmRender::RenderObject &ro = world->m_RenderObjects[ro_count_begin + i];
        FillRenderObject(world, render_context, ro, first->m_RenderConstants,
                         texture, material,
                         SpineBlendModeToRenderBlendMode(
                             (spBlendMode)scratch_draw_descs[i].m_BlendMode),
    ...
    FillRenderObject(world, render_context, ro, first->m_RenderConstants,
                     texture, material, blend_mode, vertex_start, vertex_count);
  }
}

static void
RenderListFrustumCulling(dmRender::RenderListVisibilityParams const &params) {
  ...
  for (uint32_t i = 0; i < num_entries; ++i) {
    ...
    bool intersect =
        dmIntersection::TestFrustumSphere(frustum, center_world, radius);
    entry->m_Visibility =
        intersect ? dmRender::VISIBILITY_FULL : dmRender::VISIBILITY_NONE;
  }
}

static void
RenderListDispatch(dmRender::RenderListDispatchParams const &params) {
  SpineModelWorld *world = (SpineModelWorld *)params.m_UserData;

  switch (params.m_Operation) {
  case dmRender::RENDER_LIST_OPERATION_BEGIN: {
    dmGraphics::SetVertexBufferData(world->m_VertexBuffer, 0, 0,
                                    dmGraphics::BUFFER_USAGE_DYNAMIC_DRAW);
  ...
  case dmRender::RENDER_LIST_OPERATION_BATCH: {
    RenderBatch(world, params.m_Context, params.m_Buf, params.m_Begin,
                params.m_End);
    break;
  }
  case dmRender::RENDER_LIST_OPERATION_END: {
    dmGraphics::SetVertexBufferData(world->m_VertexBuffer,
                                    sizeof(dmSpine::SpineVertex) *
                                        world->m_VertexBufferData.Size(),
                                    world->m_VertexBufferData.Begin(),
                                    dmGraphics::BUFFER_USAGE_DYNAMIC_DRAW);
    // DM_COUNTER("SpineVertexBuffer", world->m_VertexBufferData.Size() *
    // sizeof(dmSpine::SpineVertex));
    break;
  }
  default:
...
dmGameObject::UpdateResult
CompSpineModelRender(const dmGameObject::ComponentsRenderParams &params) {
  SpineModelContext *context = (SpineModelContext *)params.m_Context;
  dmRender::HRenderContext render_context = context->m_RenderContext;
  SpineModelWorld *world = (SpineModelWorld *)params.m_World;
  ...
  dmRender::RenderListEntry *render_list =
      dmRender::RenderListAlloc(render_context, count);
  dmRender::HRenderListDispatch dispatch = dmRender::RenderListMakeDispatch(
      render_context, &RenderListDispatch, &RenderListFrustumCulling, world);
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    write_ptr->m_WorldPosition =
        Point3(trans.getX(), trans.getY(), trans.getZ());
    write_ptr->m_UserData = (uintptr_t)i;
    write_ptr->m_BatchKey = component.m_MixedHash;
    write_ptr->m_TagListKey =
        dmRender::GetMaterialTagListKey(GetMaterial(&component));
...
static bool
CompSpineModelGetConstantCallback(void *user_data, dmhash_t name_hash,
                                  dmRender::Constant **out_constant) {
  SpineModelComponent *component = (SpineModelComponent *)user_data;
  return component->m_RenderConstants &&
         dmGameSystem::GetRenderConstant(component->m_RenderConstants,
                                         name_hash, out_constant);
}

static void
CompSpineModelSetConstantCallback(void *user_data, dmhash_t name_hash,
                                  int32_t value_index, uint32_t *element_index,
                                  const dmGameObject::PropertyVar &var) {
  SpineModelComponent *component = (SpineModelComponent *)user_data;
  if (!component->m_RenderConstants)
    component->m_RenderConstants = dmGameSystem::CreateRenderConstants();
  dmGameSystem::SetRenderConstant(component->m_RenderConstants,
                                  GetMaterial(component), name_hash,
                                  value_index, element_index, var);
  component->m_ReHash = 1;
}

bool CompSpineModelPlayAnimation(SpineModelComponent *component,
                                 dmGameSystemDDF::SpinePlayAnimation *message,
                                 dmMessage::URL *sender,
                                 dmScript::LuaCallbackInfo *callback_info,
                                 lua_State *L) {
  bool result = PlayAnimation(component, message->m_AnimationId,
                              (dmGameObject::Playback)message->m_Playback,
                              message->m_BlendDuration, message->m_Offset,
                              message->m_PlaybackRate, message->m_Track - 1);
  if (result) {
    SpineAnimationTrack &track =
        component->m_AnimationTracks[message->m_Track - 1];
...
bool CompSpineModelCancelAnimation(
    SpineModelComponent *component,
    dmGameSystemDDF::SpineCancelAnimation *message) {
...
bool CompSpineModelSetConstant(SpineModelComponent *component,
                               dmGameSystemDDF::SetConstant *message) {
  dmGameObject::PropertyResult result = dmGameSystem::SetMaterialConstant(
      GetMaterial(component), message->m_NameHash,
      dmGameObject::PropertyVar(message->m_Value), message->m_Index,
      CompSpineModelSetConstantCallback, component);
  return result == dmGameObject::PROPERTY_RESULT_OK;
}

bool CompSpineModelResetConstant(SpineModelComponent *component,
                                 dmGameSystemDDF::ResetConstant *message) {
  if (component->m_RenderConstants) {
    component->m_ReHash |= dmGameSystem::ClearRenderConstant(
        component->m_RenderConstants, message->m_NameHash);
  }
  return true;
}

dmGameObject::UpdateResult
CompSpineModelOnMessage(const dmGameObject::ComponentOnMessageParams &params) {
  SpineModelWorld *world = (SpineModelWorld *)params.m_World;
  SpineModelComponent *component =
      GetComponentFromIndex(world, *params.m_UserData);
  if (params.m_Message->m_Id ==
      dmGameObjectDDF::Enable::m_DDFDescriptor->m_NameHash) {
    component->m_Enabled = 1;
  } else if (params.m_Message->m_Id ==
...
static bool OnResourceReloaded(SpineModelWorld *world,
                               SpineModelComponent *component, int index) {
...
void CompSpineModelOnReload(
    const dmGameObject::ComponentOnReloadParams &params) {
  SpineModelWorld *world = (SpineModelWorld *)params.m_World;
...
dmGameObject::PropertyResult CompSpineModelGetProperty(
    const dmGameObject::ComponentGetPropertyParams &params,
  ...
  if (params.m_PropertyId == PROP_SKIN) {
    spSkin *skin = component->m_SkeletonInstance
                       ->skin; // ? component->m_SkeletonInstance->skin :
                               // component->m_SkeletonInstance->defaultSkin;
    ...
    SpineAnimationTrack *track =
        GetTrackFromIndex(component, params.m_Options.m_Index);
    if (track && track->m_AnimationInstance) {
    ...
    dmRender::HMaterial material = GetMaterial(component);
    return dmGameSystem::GetResourceProperty(context->m_Factory, material,
                                             out_value);
  }
  return dmGameSystem::GetMaterialConstant(
      GetMaterial(component), params.m_PropertyId, params.m_Options.m_Index,
      out_value, false, CompSpineModelGetConstantCallback, component);
}

dmGameObject::PropertyResult CompSpineModelSetProperty(
    const dmGameObject::ComponentSetPropertyParams &params) {
  SpineModelContext *context = (SpineModelContext *)params.m_Context;
  SpineModelWorld *world = (SpineModelWorld *)params.m_World;
  SpineModelComponent *component =
      GetComponentFromIndex(world, *params.m_UserData);
  if (params.m_PropertyId == PROP_SKIN) {
    ...
    float duration = track->m_AnimationInstance->animationEnd -
                     track->m_AnimationInstance->animationStart;
    ...
    dmGameObject::PropertyResult res = dmGameSystem::SetResourceProperty(
        context->m_Factory, params.m_Value, MATERIAL_EXT_HASH,
        (void **)&component->m_Material);
    component->m_ReHash |= res == dmGameObject::PROPERTY_RESULT_OK;
    return res;
  }
  return dmGameSystem::SetMaterialConstant(
      GetMaterial(component), params.m_PropertyId, params.m_Value,
      params.m_Options.m_Index, CompSpineModelSetConstantCallback, component);
}

static void
ResourceReloadedCallback(const dmResource::ResourceReloadedParams &params) {
  // E.g. if the Spine json or atlas has changed, we may need to update things
  // here

  // SpineModelWorld* world = (SpineModelWorld*) params.m_UserData;
  // dmArray<SpineModelComponent*>& components =
  // world->m_Components.GetRawObjects(); uint32_t n = components.Size(); for
  // (uint32_t i = 0; i < n; ++i)
  // {
  //     SpineModelComponent* component = components[i];
  //     if (component->m_Resource != 0x0 && component->m_Resource->m_RigScene
  //     == params.m_Resource->m_Resource)
  //         OnResourceReloaded(world, component, i);
  // }
}

static dmGameObject::Result
CompTypeSpineModelCreate(const dmGameObject::ComponentTypeCreateCtx *ctx,
                         dmGameObject::ComponentType *type) {
  SpineModelContext *spinemodelctx = new SpineModelContext;
  spinemodelctx->m_Factory = ctx->m_Factory;
  spinemodelctx->m_GraphicsContext =
      *(dmGraphics::HContext *)ctx->m_Contexts.Get(dmHashString64("graphics"));
  spinemodelctx->m_RenderContext =
      *(dmRender::HRenderContext *)ctx->m_Contexts.Get(
          dmHashString64("render"));

  int32_t max_rig_instance =
      dmConfigFile::GetInt(ctx->m_Config, "rig.max_instance_count", 128);
  spinemodelctx->m_MaxSpineModelCount =
      dmMath::Max(dmConfigFile::GetInt(ctx->m_Config, "spine.max_count", 128),
                  max_rig_instance);
  ...
  // We sould be able to avoid doing UpdateTransforms again in the Render()
  // function
  // ComponentTypeSetPrio(type, 1300);
  ComponentTypeSetPrio(
      type, 350); // 400 is the collision objects, and others come after that
...
static dmGameObject::Result
CompTypeSpineModelDestroy(const dmGameObject::ComponentTypeCreateCtx *ctx,
                          dmGameObject::ComponentType *type) {
  SpineModelContext *spinemodelctx =
      (SpineModelContext *)ComponentTypeGetContext(type);
  delete spinemodelctx;
  return dmGameObject::RESULT_OK;
}

static dmGameObject::Result
ComponentType_Destroy(const dmGameObject::ComponentTypeCreateCtx *ctx,
                      dmGameObject::ComponentType *type) {
  SpineModelContext *spinemodelctx =
      (SpineModelContext *)ComponentTypeGetContext(type);
  delete spinemodelctx;
...
bool CompSpineModelSetIKTargetInstance(SpineModelComponent *component,
                                       dmhash_t constraint_id, float mix,
                                       dmhash_t instance_id) {
  if (instance_id == 0) {
  ...
  spIkConstraint *constraint =
      component->m_SkeletonInstance->ikConstraints[*index];

  IKTarget target;
  target.m_ConstraintHash =
      constraint_id; // for removing a constraint from this list
  target.m_Constraint = component->m_SkeletonInstance->ikConstraints[*index];
  target.m_Position = dmVMath::Point3(0, 0, 0); // unused
  target.m_Target = dmGameObject::GetInstanceFromIdentifier(
...
bool CompSpineModelSetIKTargetPosition(SpineModelComponent *component,
                                       dmhash_t constraint_id, float mix,
                                       Point3 position) {
  SpineModelResource *spine_model = component->m_Resource;
  SpineSceneResource *spine_scene = spine_model->m_SpineScene;
...
bool CompSpineModelResetIKTarget(SpineModelComponent *component,
                                 dmhash_t constraint_id) {
...
bool CompSpineModelSetAttachment(SpineModelComponent *component,
                                 dmhash_t slot_id, dmhash_t attachment_id) {
  SpineModelResource *spine_model = component->m_Resource;
  SpineSceneResource *spine_scene = spine_model->m_SpineScene;
  ...
  // SP_API int spSkeleton_setAttachment(spSkeleton *self, const char *slotName,
  // const char *attachmentName);

  const char *attachment_name = 0;
  if (attachment_id) {
    const char **p_attachment_name =
        spine_scene->m_AttachmentHashToName.Get(attachment_id);
    if (!p_attachment_name) {
      dmLogError("No attachment named '%s'",
                 dmHashReverseSafe64(attachment_id));
      return false;
    }
  ...
  // it's a bit weird to use strings here, but we'd rather not use too much
  // knowledge about the internals
  return 1 == spSkeleton_setAttachment(component->m_SkeletonInstance,
                                       slot->data->name, attachment_name);
}

bool CompSpineModelGetBone(SpineModelComponent *component, dmhash_t bone_name,
                           dmhash_t *instance_id) {
...
DM_DECLARE_COMPONENT_TYPE(ComponentTypeSpineModelExt, "spinemodelc",
                          dmSpine::CompTypeSpineModelCreate,
                          dmSpine::CompTypeSpineModelDestroy);
